...
results <- read_csv("cohort_adj_results.csv")
results$`Hazard ratio (95% CI)` <- gsub("to", "—", results$`Hazard ratio (95% CI)`)
...
results$`Forest plot` <- "                         "
results <- results |> relocate(`Hazard ratio (95% CI)`, .after = `Forest plot`  )
...
plot <- forest(
      ...
      est = results$hr, # the main effect
...
ggsave(
      filename = "sensitivity_analysis.svg",   # your filename
